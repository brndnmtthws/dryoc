...
//! | `src/scalarmult_curve25519.rs`, `src/edwards25519/mod.rs` and `src/fe25519/mod.rs` Curve25519 BMI2 roots | Always available on `x86_64` | The X25519 ladder, `mul_base`, `double_scalar_mul_basepoint_vartime`, `Fe::invert` and `Fe::sqrt_ratio_i` each call a `#[target_feature(enable = "bmi2")]` copy of the same safe, inlined arithmetic through its token wrapper when a `Bmi2` token can be constructed, so the `u128` field products compile to `mulx`. These functions contain no intrinsics or `asm!`; the only unsafe operation is the wrapper's call. |
...
//! | `src/keccak/keccak_aarch64.rs` 1- and 2-way Keccak-p\[1600\] SHA3-extension permutation | Always available on little-endian `aarch64` | `Sponge::permute` and `permute_lanes` (behind `ParSponge`) call the `#[target_feature(enable = "neon,sha3")]` functions `permute1_sha3_unchecked`/`permute2_sha3_unchecked` through the token wrappers of the `Sha3` token held by its `Kernel` handle. The kernel uses only safe value intrinsics and reads and writes the states through their `&mut [u64; 25]` references (disjoint ones from `get_disjoint_mut` for two states). |
