...
//! | `src/argon2/argon2_x86_64.rs` Argon2 AVX2 and AVX-512 block compression | `alloc` on `x86_64` | Calls a `#[target_feature(enable = "avx2")]` or `#[target_feature(enable = "avx512f")]` `fill_block` through a `Kernel` handle that can only be constructed after `has_x86_feature!` confirms that feature. The kernels use safe value intrinsics; block words are loaded and stored through `x86_64::load_words`/`store_words`/`load_words512`/`store_words512` (`_mm256_loadu_si256`/`_mm256_storeu_si256`/`_mm512_loadu_si512`/`_mm512_storeu_si512` on `&[u64; 4]`/`&[u64; 8]` references, shared or exclusive as the operation needs, which guarantee exactly 32 or 64 readable or writable bytes and need no alignment). |
...
//! | `src/sha512/sha512_aarch64.rs` SHA-512 two-state hardware compression | Always available on little-endian `aarch64` | `Sha512::from_blocks` calls the `#[target_feature(enable = "sha3")]` function `compress2` only after `has_aarch64_feature!("sha3")` succeeds. Its single `asm!` block compresses one block into each of two independent states with the two round sequences interleaved (the HMAC inner and outer key blocks); it reads the two `&[u8; 128]` blocks and the `K64` table, reads and writes both eight-word states through their `&mut` pointers, clobbers every vector register and `x3`, and touches no stack. |
//! | `src/edwards25519/edwards25519_x86_64.rs` basepoint table lookup | Always available on `x86_64` | `select_row` calls the `#[target_feature(enable = "avx512f")]` function `edwards25519_x86_64::select_row` only after `has_x86_feature!("avx512f")` succeeds; the function uses safe value intrinsics (a vector compare of the digit and masked blends over every entry) and writes its result through `x86_64::store_words512`, reading every table entry regardless of the digit. |
//! | `src/edwards25519/edwards25519_neon.rs` basepoint table lookup | Always available on `aarch64` | `select_row` calls the `#[target_feature(enable = "neon")]` function `edwards25519_neon::select_row` only after `has_aarch64_feature!("neon")` succeeds; the function uses only safe value intrinsics (`and`/`orr` on vectors built from limbs) and reads every table entry regardless of the digit. |
