...
//! | `src/chacha20/chacha20_neon.rs` ChaCha20 NEON and SVE2 backends | Always available on little-endian `aarch64` | Calls a `#[target_feature(enable = "neon")]` keystream kernel through a `Kernel` handle that can only be constructed after `is_aarch64_feature_detected!("neon")` (or `"sve2"`) succeeds at runtime. The NEON kernel uses only safe intrinsics and safe slice loads and stores. The SVE2 kernel additionally runs the ChaCha20 rounds in one `asm!` block of `add`/`xar` instructions on the 32 vector registers, bound as `inout` operands with no memory access, called only when `"sve2"` was detected. |
