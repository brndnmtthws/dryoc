...
//! padding happens and which domain byte it uses. On AArch64 its
//! permutation is the in-crate lazy-rotation scalar code in `keccak_soft.rs`
//! (measured faster on Neoverse V3 than the SHA3-extension kernel with one
//! state), and otherwise the RustCrypto `keccak` crate. [`ParSponge`]
//! runs several independent sponges together: on AArch64 with the SHA3
//! extension three at a time with 24 rounds (two in NEON vectors beside one
//! on the integer registers, `keccak3_aarch64.rs`, generated by
//! `keccak3_aarch64.py`) and two at a time otherwise (`keccak_aarch64.rs`),
//! with a [`Chain`] companion's permutations taking slots in those calls;
//! four at a time through the AVX2 kernel in `keccak_x86_64.rs` when an
//! x86-64 CPU has AVX2, two at a time through the `simd128` kernel in
//! `keccak_wasm32.rs` when the crate is built for WebAssembly with
//! `simd128`, and the rest through the crate.
...
impl<const RATE: usize, const ROUNDS: usize> Sponge<RATE, ROUNDS> {
    ...
    /// permuted in place and wiped on drop. On AArch64 the in-crate scalar
    /// code runs instead of the `keccak` crate's AArch64 backend, whose
    /// single-state path leaves an unwiped `[state, zero]` copy (see
    /// `keccak_aarch64.rs`).
    fn permute(&mut self) {
        #[cfg(target_arch = "aarch64")]
        keccak_soft::permute::<ROUNDS>(&mut self.state);
        #[cfg(not(target_arch = "aarch64"))]
        {
    ...
    pub(crate) fn absorb(&mut self, mut input: &[u8]) {
        while !input.is_empty() {
            if self.offset == RATE {
                // A full block after a full block: XORed in by the
                // permutation's final stores, the rate staying full.
                #[cfg(target_arch = "aarch64")]
                if let Some((block, rest)) = input.split_at_checked(RATE) {
                    keccak_soft::permute_xor::<ROUNDS>(&mut self.state, block);
                    input = rest;
                    continue;
                }
                self.permute();
...
/// A single sponge that a [`ParSponge`] advances alongside its own lanes:
/// its permutations, one per call, take slots in the multi-state calls of
/// the lanes' permutation steps, first the slots the lanes leave free and
/// then, where its [`quota`](Self::quota) asks for more, slots of extra
/// calls. Only the AArch64 kernel advances it (see [`permute_lanes`]);
/// elsewhere the companion is never advanced and finishes on its own.
pub(crate) trait Companion {
    /// Absorbs up to the next due permutation and returns the state to
    /// permute, or `None` when none is left. Until [`permuted`](Self::permuted)
    /// is called, further calls return the same state unchanged.
    fn pending(&mut self) -> Option<&mut [u64; 25]>;

    /// Records that the state [`pending`](Self::pending) returned has been
    /// permuted.
    fn permuted(&mut self);

    /// The permutations to take along at the permutation step asking (each
    /// step with lanes asks once), even at the cost of extra calls.
    #[cfg(all(target_arch = "aarch64", target_endian = "little", not(miri)))]
    fn quota(&mut self) -> usize;
}

/// [`hash`] of the concatenation of `parts` as a [`Companion`], so its
/// permutations can share a [`ParSponge`]'s calls; [`finish`](Self::finish)
/// runs the rest alone and squeezes the output. The permutations are the
/// ones [`hash`] runs, in the same order, so the output is identical.
pub(crate) struct Chain<'a, const RATE: usize> {
    sponge: Sponge<RATE, ROUNDS_FULL>,
    /// Permutations not yet run.
    due: usize,
    /// Permutation steps expected to ask for a quota, not yet asked (read
    /// only by the AArch64 kernel's scheduling).
    #[cfg_attr(
        not(all(target_arch = "aarch64", target_endian = "little", not(miri))),
        allow(dead_code)
    )]
    steps: usize,
    /// Unabsorbed rest of the current part, then the remaining parts.
    input: &'a [u8],
    parts: &'a [&'a [u8]],
    domain: u8,
    /// Padding has been applied; its permutation is pending or done.
    padded: bool,
    done: bool,
}

impl<'a, const RATE: usize> Chain<'a, RATE> {
    /// The hash of `parts`, expecting `steps` permutation steps to take its
    /// permutations along, which it spreads evenly over them (a scheduling
    /// hint only: the output is the same whatever the steps).
    pub(crate) fn new(domain: u8, parts: &'a [&'a [u8]], steps: usize) -> Self {
        debug_assert!((0x01..=0x7f).contains(&domain));
        // One permutation per full rate of input and one after the padding.
    ...
    /// Runs the permutations still due and fills `output`; the chain is
    /// spent afterwards and only dropped.
    pub(crate) fn finish(&mut self, output: &mut [u8]) {
...
impl<const RATE: usize> Companion for Chain<'_, RATE> {
    fn pending(&mut self) -> Option<&mut [u64; 25]> {
        if self.done {
            return None;
        }
        if !self.padded {
            // `Sponge::absorb`: a full rate is permuted only once more
            // input follows.
            loop {
                if self.input.is_empty() {
                    let Some((next, rest)) = self.parts.split_first() else {
                        break;
                    };
                    (self.input, self.parts) = (next, rest);
                    continue;
                }
                if self.sponge.offset == RATE {
                    return Some(&mut self.sponge.state);
                }
                let take = self.input.len().min(RATE - self.sponge.offset);
                let (chunk, rest) = self.input.split_at(take);
                xor_into(&mut self.sponge.state, self.sponge.offset, chunk);
                self.sponge.offset += take;
                self.input = rest;
            }
            // `Sponge::pad`.
            if self.sponge.offset == RATE {
            ...
            xor_into(&mut self.sponge.state, self.sponge.offset, &[self.domain]);
            xor_into(&mut self.sponge.state, RATE - 1, &[0x80]);
            self.padded = true;
        }
        Some(&mut self.sponge.state)
    }

    fn permuted(&mut self) {
        self.sponge.offset = 0;
        self.done = self.padded;
        self.due -= 1;
    }

    #[cfg(all(target_arch = "aarch64", target_endian = "little", not(miri)))]
    fn quota(&mut self) -> usize {
        let quota = self.due.div_ceil(self.steps.max(1));
...
/// a multi-state permutation (such as the AArch64 SHA3-extension kernels)
/// runs several of them per call.
///
...
impl<const RATE: usize, const ROUNDS: usize, const N: usize> ParSponge<RATE, ROUNDS, N> {
    ...
    /// rate, taking `companion`'s permutations along with 24 rounds (see
    /// [`Companion`]).
    fn permute(&mut self, selected: [bool; N], companion: &mut Option<&mut (dyn Companion + '_)>) {
        for (offset, _) in self.offsets.iter_mut().zip(selected).filter(|(_, s)| *s) {
            *offset = 0;
        }
        let companion = if ROUNDS == ROUNDS_FULL {
            companion.as_deref_mut()
        } else {
            None
        };
        permute_lanes::<ROUNDS, N>(&self.keccak, &mut self.states, selected, companion);
    }
    ...
    pub(crate) fn absorb(&mut self, mut inputs: [&[u8]; N]) {
        loop {
            let selected =
                core::array::from_fn(|i| !inputs[i].is_empty() && self.offsets[i] == RATE);
            self.permute(selected, &mut None);
            let mut done = true;
    ...
    /// [`Sponge::pad`] on every lane, advancing `companion` alongside (see
    /// [`Companion`]).
    pub(crate) fn pad_with(
        &mut self,
        domain: u8,
        mut companion: Option<&mut (dyn Companion + '_)>,
    ) {
        debug_assert!((0x01..=0x7f).contains(&domain));
        self.permute(self.offsets.map(|offset| offset == RATE), &mut companion);
        for (state, &offset) in self.states.iter_mut().zip(&self.offsets) {
            xor_into(state, offset, &[domain]);
            xor_into(state, RATE - 1, &[0x80]);
        }
        self.permute([true; N], &mut companion);
    }

    /// [`Sponge::pad`] on every lane with that lane's domain byte, the lanes
    /// that need them permuted together.
    pub(crate) fn pad_lanes(&mut self, domains: [u8; N]) {
        debug_assert!(domains.iter().all(|d| (0x01..=0x7f).contains(d)));
        self.permute(self.offsets.map(|offset| offset == RATE), &mut None);
        for ((state, &offset), domain) in self.states.iter_mut().zip(&self.offsets).zip(domains) {
            xor_into(state, offset, &[domain]);
            xor_into(state, RATE - 1, &[0x80]);
        }
        self.permute([true; N], &mut None);
    }

    /// Fills `outputs[i]` from the rate of lane `i`, permuting between
    /// blocks; lanes given an empty output are left as they are.
    pub(crate) fn squeeze(&mut self, outputs: [&mut [u8]; N]) {
        self.squeeze_with(outputs, None);
    }

    /// [`squeeze`](Self::squeeze), advancing `companion` alongside (see
    /// [`Companion`]).
    pub(crate) fn squeeze_with(
        &mut self,
        mut outputs: [&mut [u8]; N],
        mut companion: Option<&mut (dyn Companion + '_)>,
    ) {
        loop {
            let selected =
                core::array::from_fn(|i| !outputs[i].is_empty() && self.offsets[i] == RATE);
            self.permute(selected, &mut companion);
            let mut done = true;
...
/// Applies Keccak-p[1600, `ROUNDS`] to each state in `selected`, and on
/// AArch64 takes `companion`'s permutations along (see [`Companion`]).
///
/// This is the one place multi-state permutations are chosen. On AArch64
/// with the SHA3 extension, the in-crate kernels permute them three at a
/// time with 24 rounds (`keccak3_aarch64.rs`, with the companion's
/// permutations in some of the calls) and two at a time otherwise
/// (`keccak_aarch64.rs`, an odd one alone). On x86-64 with AVX2, the
...
fn permute_lanes<const ROUNDS: usize, const N: usize>(
    ...
    states: &mut [[u64; 25]; N],
    selected: [bool; N],
    companion: Option<&mut (dyn Companion + '_)>,
) {
    #[cfg(all(target_arch = "aarch64", target_endian = "little", not(miri)))]
    if let Some(kernel) = keccak_aarch64::detect() {
        let mut selected = selected;
        kernel.permute_selected::<ROUNDS, N>(states, &mut selected, companion);
        return;
    }
    // Only the AArch64 kernel advances a companion.
    let _ = companion;
    #[cfg(target_arch = "x86_64")]
    let selected = {
        let mut selected = selected;
...
mod tests {
    ...
    fn check_par_sponge_matches_sponge<const RATE: usize, const ROUNDS: usize, const N: usize>() {
        ...
        for round in 0..lengths.len() {
            ...
            par.absorb(core::array::from_fn(|lane| &inputs[lane][splits[lane]..]));
            // Odd rounds pad each lane with its own domain.
            let domains: [u8; N] = core::array::from_fn(|lane| {
                if round % 2 == 1 && lane % 2 == 1 {
                    DOMAIN_SHA3
                } else {
                    DOMAIN_SHAKE
                }
            ...
            if round % 2 == 1 {
                par.pad_lanes(domains);
            } else {
                par.pad_with(DOMAIN_SHAKE, None);
            }
    ...
    /// A [`Chain`] gives exactly [`hash`]'s output whether it finishes alone
    /// or has some of its permutations taken along by a [`ParSponge`], with
    /// quotas from a range of expected steps, for messages around rate
    /// multiples split into parts (with empty ones), and the lanes are
    /// unaffected by it.
    #[test]
    fn test_chain_companion_matches_hash() {
        let data: Vec<u8> = (0..1200u32).map(|i| (i * 7 + 3) as u8).collect();
        for len in [0, 1, 135, 136, 137, 271, 272, 273, 1120, 1184] {
            let (a, b) = data[..len].split_at(len / 3);
            let parts: [&[u8]; 4] = [&[], a, &[], b];
            let mut expected = [0u8; 32];
            hash::<RATE_256>(&mut expected, DOMAIN_SHA3, &[&data[..len]]);

            let mut alone = [0u8; 32];
            Chain::<RATE_256>::new(DOMAIN_SHA3, &parts, 0).finish(&mut alone);
            assert_eq!(alone, expected, "alone, len {len}");

            for (blocks, steps) in [1, 3, 12]
                .into_iter()
                .flat_map(|b| [(b, 0), (b, 2), (b, 99)])
            {
                let mut chain = Chain::<RATE_256>::new(DOMAIN_SHA3, &parts, steps);
                let mut par = ParSponge::<RATE_128, ROUNDS_FULL, 3>::new();
                par.absorb([b"x", b"yy", b"zzz"]);
                par.pad_with(DOMAIN_SHAKE, Some(&mut chain));
                let mut outputs = [[0u8; 12 * RATE_128]; 3];
                let lanes = outputs.each_mut().map(|o| &mut o[..blocks * RATE_128]);
                par.squeeze_with(lanes, Some(&mut chain));
                let mut got = [0u8; 32];
                chain.finish(&mut got);
                assert_eq!(
                    got, expected,
                    "companion, len {len}, {blocks} blocks, {steps} steps"
                );

                for (i, (output, input)) in
                    outputs.iter().zip([&b"x"[..], b"yy", b"zzz"]).enumerate()
                {
                    let mut sponge = Sponge::<RATE_128, ROUNDS_FULL>::new();
                    sponge.absorb(input);
                    sponge.pad(DOMAIN_SHAKE);
                    let mut lane = [0u8; 12 * RATE_128];
                    sponge.squeeze(&mut lane[..blocks * RATE_128]);
                    assert_eq!(
                        output[..],
                        lane[..],
                        "lane {i}, len {len}, {blocks} blocks, {steps} steps"
                    );
                }
            }
        }
    ...
    /// The AArch64 SHA3-extension kernels permute like the `keccak` crate:
    /// two states with 24 and 12 rounds, three with 24, and
    /// `permute_selected` every selected state in consecutive and scattered
    /// lanes, clearing every flag. With 12 rounds it runs no companion; with
    /// 24 a companion's state is permuted as many times as it is taken
    /// along: to fill the free slots, or its quota, at most once per call,
    /// and never beyond the permutations it has left.
    #[cfg(all(target_arch = "aarch64", target_endian = "little", not(miri)))]
    #[test]
    fn test_aarch64_sha3_permute_matches_crate() {
        ...
        impl Companion for Counted {
            fn pending(&mut self) -> Option<&mut [u64; 25]> {
                (self.taken < self.left).then_some(&mut self.state)
            }
        ...
        let initial: [[u64; 25]; 10] = core::array::from_fn(|_| core::array::from_fn(|_| next()));
        let keccak = keccak::Keccak::new();
        let permuted = |rounds_full: bool, mut state: [u64; 25], times: usize| {
            for _ in 0..times {
                if rounds_full {
                    keccak.with_p1600::<ROUNDS_FULL>(|p1600| p1600(&mut state));
                } else {
                    keccak.with_p1600::<ROUNDS_TURBO>(|p1600| p1600(&mut state));
                }
            }
            state
        };
        // Lanes, companion quota and permutations left, and the expected
        // number taken along with 24 rounds.
        let cases: [(&[usize], usize, usize, usize); 10] = [
            ...
            (&[6], 5, 9, 1),
            (&[3, 7], 0, 9, 1),
            // The second call is reserved for the companion, which has run
            // out.
            (&[3, 7], 2, 1, 1),
            (&[], 2, 9, 0),
        ];
        for kernel in keccak_aarch64::Kernel::all() {
            for rounds_full in [true, false] {
                let [mut a, mut b, mut c] = [initial[0], initial[1], initial[2]];
                if rounds_full {
                    kernel.permute2::<ROUNDS_FULL>(&mut a, &mut b);
                } else {
                    kernel.permute2::<ROUNDS_TURBO>(&mut a, &mut b);
                }
                let expected = [0, 1].map(|i| permuted(rounds_full, initial[i], 1));
                assert_eq!([a, b], expected, "two states, 24 rounds: {rounds_full}");
                if rounds_full {
                    let [mut a, mut b] = [initial[0], initial[1]];
                    kernel.permute3(&mut a, &mut b, &mut c);
                    let expected = [0, 1, 2].map(|i| permuted(true, initial[i], 1));
                    assert_eq!([a, b, c], expected, "three states");
                }
                for (lanes, quota, left, taken) in cases {
                    let mut states: [[u64; 25]; 9] = core::array::from_fn(|i| initial[i]);
                    let mut selected = [false; 9];
                    for &lane in lanes {
                        selected[lane] = true;
                    }
                    let mut companion = Counted {
                        state: initial[9],
                        left,
                        quota,
                        taken: 0,
                    ...
                    if rounds_full {
                        kernel.permute_selected::<ROUNDS_FULL, 9>(
                            &mut states,
                            &mut selected,
                            Some(&mut companion),
                        );
                        assert_eq!(companion.taken, taken, "{lanes:?} {quota} {left}");
                    } else {
                        kernel.permute_selected::<ROUNDS_TURBO, 9>(
                            &mut states,
                            &mut selected,
                            None,
                        );
                    ...
                    assert_eq!(
                        companion.state,
                        permuted(rounds_full, initial[9], companion.taken),
                        "{lanes:?}, companion"
                    );
                    for (lane, state) in states.iter().enumerate() {
                        let times = usize::from(lanes.contains(&lane));
                        assert_eq!(
                            *state,
                            permuted(rounds_full, initial[lane], times),
                            "24 rounds: {rounds_full}, {lanes:?}, lane {lane}"
                        );
                    }
                    assert_eq!(selected, [false; 9]);
                }
            }
        }
    }
