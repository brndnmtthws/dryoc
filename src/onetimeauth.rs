...
//! let mac = OnetimeAuth::compute_to_vec(&key, b"Data to authenticate");
//!
//! // Verify the mac
...
impl OnetimeAuth {
    ...
    /// message authentication code for `input` using `key`.
    pub fn compute<
    ...
    /// expected code for `key` and `input`.
...
mod tests {
    ...
    fn test_single_part() {
        ...
        OnetimeAuth::compute_and_verify(&mac, &key, b"Data to authenticate")
