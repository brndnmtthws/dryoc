...
compression functions, and `PwHash::into_parts` transferring ownership of the
hash without invoking its drop-time zeroization. CPU features are detected at
runtime with the `std` feature and taken from the compile-time target features
without it.
