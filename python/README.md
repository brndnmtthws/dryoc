...
- **Any bytes-like input.** `bytes`, `bytearray`, `memoryview`, `array`,
  ...
  another thread cannot change them mid-operation. An exact `bytearray` (not
  a subclass) is copied atomically with respect to its own methods, even on
  free-threaded CPython; writing to other buffers (a `bytearray` subclass,
  `memoryview`, `array`, NumPy array) from another thread while a call copies
  them is a race, as with `hashlib`.
